...
int main()
{
   ...
   BinTreePointer ARoot, Node1, Node2, Result;
   ...
   printf("Enter number of elements for the tree: ");
    ...
    do{
        ...
        getchar();
        AnItem = Element;
        RecBSTSearch(ARoot, AnItem, &Found, &Node1);
        printf("Give second element of BST:");
        scanf("%d", &Element);
        getchar();
        AnItem = Element;
        if(Found == TRUE){
            RecBSTSearch(ARoot, AnItem, &Found, &Node2);
        }
    }while(!Found);

    Result = LCANode(ARoot, Node1, Node2);
    printf("LCA node of %d and %d is node with element %d", Node1->Data, Node2->Data, Result->Data);
...
BinTreePointer LCANode(BinTreePointer Root, BinTreePointer node1, BinTreePointer node2){
    ...
    if ((Root->Data > node1->Data) && (Root->Data > node2->Data)){
            result = LCANode(Root->LChild, node1, node2);
            return result;
    }
    if ((Root->Data < node1->Data) && (Root->Data < node2->Data)){
            result = LCANode(Root->RChild, node1, node2);
            return result;
    }
    return Root;
}
