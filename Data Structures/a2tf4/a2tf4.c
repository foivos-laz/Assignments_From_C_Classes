...
void DiaforaLists(ListPointer List, ListPointer BList, ListPointer *FList);

int main()
{
   ListPointer AList1, PredPtr1, AList2, PredPtr2;
   ...
   for(i = 1; i <= Size1; i++){
    ...
    scanf("%d", &item);
    getchar();
    LinkedInsert(&AList1, item, PredPtr1);
   }
   ...
   for(i = 1; i <= Size2; i++){
    ...
    scanf("%d", &item);
    getchar();
    LinkedInsert(&AList2, item, PredPtr2);
   }

   printf("1h lista\n");
   LinkedTraverse(AList1);
   printf("\n");

   printf("2h lista\n");
   LinkedTraverse(AList2);
   printf("\n");
   ...
   CreateList(&FinalList);
    ...
    DiaforaLists(AList1, AList2, &FinalList);
...
void DiaforaLists(ListPointer List, ListPointer BList, ListPointer *FList){
    ListPointer PredPtrF, PredPtrB, CurrPtrA;
    ListElementType item;
    boolean Found;
    ...
    while(CurrPtrA != NULL){
        item = (*CurrPtrA).Data;
        LinearSearch(BList, item, &PredPtrB, &Found);
        if (Found == FALSE){
            LinkedInsert(FList, item, PredPtrF);
        }
        /*PredPtrA = CurrPtrA;*/
