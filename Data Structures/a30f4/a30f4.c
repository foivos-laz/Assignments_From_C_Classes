...
int main()
{
    StackElementType AnItemS;
    StackType AStack;
    ListPointer AList;
    NodeType Node[NumberOfNodes];
    ListPointer FreePtr,PredPtr;
    ListElementType AnItemL;
    int i;
    float min;
    ...
    for(i=1;i<=5;i++){
        ...
        scanf("%f", &AnItemL.Grade);
        getchar();
        Insert(&AList, Node, &FreePtr, PredPtr, AnItemL);
    }
    printf("Question F\nStorage pool\n");
    printAll(AList, FreePtr, Node);
    ...
    printf("Question H\n");
    if(EmptyList(FreePtr)) printf("Empty List\n");
...
float FindMins(ListPointer List, NodeType Node[NumberOfNodes], StackType *Stack){
    ...
    if (!EmptyList(List)){
        CurrPtr =List;
        min = Node[List].Data.Grade;
        while (CurrPtr != NilValue)
        {
            if(Node[CurrPtr].Data.Grade == min){
                Item = (StackElementType) Node[CurrPtr].Data.AM;
                Push(Stack, Item);
            }
            else if(Node[CurrPtr].Data.Grade < min){
