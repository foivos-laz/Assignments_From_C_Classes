...
double FindMax(int additionPerCol[], int *maxTime);
void PrintAll(double bestTimeStart, double bestTimeEnd, int NumberOfCelebrities, int Celebrities[NumberOfCelebrities][6], char CelebrityNames[NumberOfCelebrities][25], int max);

int main(){
    int numberOfCelebrities, MaxTime;
    ...
    bestTimeEnd = FindMax(additionPerCol, &MaxTime);

    bestTimeStart = bestTimeEnd - 1.00;

    /*Print the final result*/
    PrintAll(bestTimeStart, bestTimeEnd, numberOfCelebrities, Celebrities, CelebrityGroupNames, MaxTime);
...
void PrintAll(double bestTimeStart, double bestTimeEnd, int NumberOfCelebrities, int Celebrities[NumberOfCelebrities][6], char CelebrityNames[NumberOfCelebrities][25],int max){
    ...
    printf("The attending celebrities are:\n");
    for (i = 0; i < NumberOfCelebrities; i++){
        if (Celebrities[i][max] == 1){
            printf("%d. %s", i, CelebrityNames[i]);
        }
    }
}
